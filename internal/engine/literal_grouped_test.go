...
func (m *literalMatcher) scanHashGrouped(data []byte, scratch *Scratch) {
	if len(data) == 0 {
		return
	}
	hashed := &m.hashed
	...
	for ; offset+8 <= len(data); offset += 8 {
		...
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset, window, key, scratch)
		}
		...
		key = literalHash(window)
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset+1, window, key, scratch)
		}
		...
		key = literalHash(window)
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset+2, window, key, scratch)
		}
		...
		key = literalHash(window)
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset+3, window, key, scratch)
		}
		...
		key = literalHash(window)
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset+4, window, key, scratch)
		}
		...
		key = literalHash(window)
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset+5, window, key, scratch)
		}
		...
		key = literalHash(window)
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset+6, window, key, scratch)
		}
		...
		key = literalHash(window)
		if hashed.present[key>>6]&(uint64(1)<<(key&63)) != 0 {
			hashed.confirmGrouped(data, offset+7, window, key, scratch)
		}
		pairKey = uint16(raw)
		if masked.present[pairKey>>6]&(uint64(1)<<(pairKey&63)) != 0 {
